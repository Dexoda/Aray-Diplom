...
//     // ОБЯЗАТЕЛЬНО передавай items и totalAmount!
//     res.render('orders/cart', {
//       title: 'Корзина',
//       items,
//       totalAmount,
//       user: req.user // если используешь user в ejs
//     });
//   } catch (error) {
//     next(error);
//   }
...
//   // Add company logo
//   doc.image(path.join(__dirname, '../public/images/logo.jpg'), 50, 45, { width: 100 })
//     .fillColor('#333')
//     .fontSize(20)
...
exports.showCheckoutForm = async (req, res, next) => {
  try {
    ...
    // Формируем массив items для шаблона
    const items = [];
    let totalAmount = 0;
    ...
    // Показываем форму оплаты — если корзина пуста, возвращаемся к товарам
    if (items.length === 0) {
      return res.redirect('/products');
    }
...
exports.createOrder = async (req, res, next) => {
  try {
    ...
    // Если корзина реально пуста (все товары удалены/кончились) — редирект
    if (items.length === 0) {
      return res.redirect('/products');
    }
...
exports.viewCart = async (req, res, next) => {
  try {
    ...
    // Даже если ошибка — передай пустые значения, чтобы не падал шаблон!
    res.render('orders/cart', {
      title: 'Корзина',
      items: [],
      totalAmount: 0,
...
function generatePdfInvoice(order, res) {
  ...
  order.items.forEach(item => {
    doc.text(item.name, 50, y, { width: 240 })
      .text(item.quantity.toString(), 300, y)
      .text(`${item.price.toFixed(2)} Т`, 350, y)
      .text(`${(item.price * item.quantity).toFixed(2)} ₽`, 400, y);
  ...
  doc.moveTo(50, y)
    ...
    .stroke()
    .moveDown()
    .text(`Итого: ${total.toFixed(2)} Т`, 400, y + 20)
    .text(`Способ оплаты: ${order.paymentMethod}`, 50, y + 20)
    .text(`Статус оплаты: ${order.isPaid ? 'Оплачено' : 'Не оплачено'}`, 50, y + 40)
    .moveDown(2);
